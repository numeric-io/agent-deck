...
def create_slack_app(config: dict):
    ...
    async def _handle_slack_text(
        ...
        API calls.
        """
        asyncio.create_task(_do_handle_slack_text(text, say, thread_ts, user_id=user_id, event_channel=event_channel))

    async def _do_handle_slack_text(text: str, say, thread_ts: str = None, user_id: str = None, event_channel: str = None):
